...
from app.src.core.tests.repositories.in_memory_sensor_data_repository import (
    InMemoryTravelOptionRepository,
)

from app.src.domain.enterprise.entities.sensor_data import TravelOption
...
def use_case(repository):
    return FindMostEconomicalTravelOptionForCityUseCase(repository)

def test_find_most_economical_with_valid_city_returns_right(use_case):
    city = "São Paulo"
    result = use_case.execute(city)
    assert result.is_right()
    assert result.right_value.name == "Linha Azul"  # Supondo que este seja o mais econômico esperado

def test_find_most_economical_with_invalid_city_returns_left(use_case):
...
def test_find_most_economical_with_empty_city_returns_left(use_case):
    # Dado
    city = ""
    # Quando
    result = use_case.execute(city)
